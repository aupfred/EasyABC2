...
from easyabc2.models.abc_assist.aligner import get_bar_length
from easyabc2.models.abc_assist.generalmidi import general_midi_instruments
from easyabc2.models.abc_assist.abc_tune import AbcTune, note_to_number, number_to_note
from easyabc2.models.abc_assist.abc_character_encoding import unicode_text_to_abc
...
class AbcAction(object):
    ...
    def html_image(self, image_name, description):
        image_path = os.path.join(self.resources_path, "img", f"{image_name}.png")
        if os.path.exists(image_path):
            return f'<img src="{image_path}" border="0" alt="{description or ""}">'
        return ''
    ...
    def add_options(self, row):
        ...
        if only_common:
            ...
            if self.show_non_common:
                url_tuple = UrlTuple(action_url, self.html_image('arrow-up', _('Less options')))
            else:
                url_tuple = UrlTuple(action_url, self.html_image('arrow-down', _('More options')))
            ...
            if value.common or self.show_non_common:
                ...
                if isinstance(value, ActionValue):
                    ...
                    if isinstance(value, ValueImageDescription):
                        image_html = self.html_image(value.image_name, desc)
...
class RemoveAction(AbcAction):
    ...
    def get_action_html(self, context):
        ...
        params = {}
        action_url = self.get_action_url(params)
        image_html = self.html_image('remove-black', desc)
        columns = []
...
class AbcActionHandlers(object):
    def __init__(self, elements, resources_path):
    ...
    def register_actions(self, actions):
        for action in actions:
            action.resources_path = self.resources_path
